...
- 共有アカウント本人向けの画面は `http://localhost:8080/account/` です。プロフィール表示、連携 app の確認/解除、共有アカウント削除予約をここで行います。
- メールアドレス、電話番号、パスワード、MFA 設定は `/settings` で Kratos self-service settings flow を使って変更します。
