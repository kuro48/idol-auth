...
:root{
  --oshi:#ff8a3d; --oshi-weak:#fff1e8; --bg:#f8f6f3; --surface:#fff;
  --surface-2:#fffaf6; --text:#26211f; --muted:#776d67; --border:#eadfd7;
  --shadow:0 16px 40px rgba(48,35,28,.1); --radius-lg:28px; --radius-md:18px;
  --danger:#e85d75; --success:#35a67b; --warning:#e9a23b; --sw:280px;
}
...
.topnav{position:fixed;top:0;left:0;right:0;z-index:100;height:64px;background:var(--surface-2);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 24px;gap:14px;backdrop-filter:blur(10px)}
.topnav-mark{width:38px;height:38px;border-radius:13px;background:var(--oshi);color:#fff;display:grid;place-items:center;font-size:18px;font-weight:900;box-shadow:0 10px 24px color-mix(in srgb,var(--oshi) 30%,transparent)}
...
.topnav-email{font-size:12px;color:var(--muted);font-weight:700;padding:8px 14px;background:var(--surface);border:1px solid var(--border);border-radius:999px}

.layout{display:flex;padding-top:64px;min-height:100vh}
.sidebar{position:fixed;top:64px;left:0;bottom:0;width:var(--sw);background:var(--surface-2);border-right:1px solid var(--border);padding:24px 18px;display:flex;flex-direction:column;gap:22px;overflow-y:auto}
.brand{display:flex;align-items:center;gap:12px;padding:4px 6px}
.brand-mark{width:44px;height:44px;border-radius:16px;background:var(--oshi);display:grid;place-items:center;color:#fff;font-size:22px;box-shadow:0 10px 24px color-mix(in srgb,var(--oshi) 32%,transparent)}
...
.nav-link{width:100%;border:0;color:var(--muted);background:transparent;padding:11px 12px;border-radius:16px;text-align:left;display:flex;align-items:center;gap:11px;font-weight:700;text-decoration:none;transition:transform 160ms ease,background 160ms ease,color 160ms ease,box-shadow 160ms ease,outline-color 160ms ease;outline:2px solid transparent;outline-offset:-2px}
...
.nav-link.active{color:var(--text);background:var(--surface);box-shadow:0 10px 24px rgba(45,35,28,.08);outline:2px solid color-mix(in srgb,var(--oshi) 38%,transparent)}
...
.sidebar-footer{margin-top:auto;padding:14px;background:var(--surface);border:1px solid var(--border);border-radius:20px;box-shadow:0 10px 24px rgba(45,35,28,.06)}
.theme-row{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px}
.theme-row strong{font-size:13px;font-weight:900}
.theme-toggle{border:1px solid var(--border);background:var(--surface-2);color:var(--text);padding:7px 12px;border-radius:999px;font-size:12px;font-weight:800;transition:transform 160ms ease}
.theme-toggle:hover{transform:translateY(-1px)}
.color-dots{display:flex;gap:8px;flex-wrap:wrap}
.color-dot{width:26px;height:26px;border-radius:50%;border:3px solid var(--surface);box-shadow:0 0 0 1px var(--border);cursor:pointer;padding:0;transition:transform 160ms ease}
...
.metric-card{padding:20px;border-radius:24px;border:1px solid var(--border);background:var(--surface)}
...
.card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:var(--shadow);padding:24px 26px;margin-bottom:22px}
...
.table-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);box-shadow:var(--shadow);overflow:hidden;margin-bottom:22px}
...
th{text-align:left;padding:12px 24px;background:var(--surface-2);border-top:1px solid var(--border);border-bottom:1px solid var(--border);font-weight:800;color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap}
...
.btn{display:inline-flex;align-items:center;gap:6px;padding:11px 18px;border-radius:16px;font-size:13px;font-weight:900;border:1px solid transparent;text-decoration:none;transition:transform 160ms ease,box-shadow 160ms ease,background 160ms ease;letter-spacing:.01em}
...
.btn-primary:hover{box-shadow:0 16px 32px color-mix(in srgb,var(--oshi) 32%,transparent)}
...
.form-row{display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;margin-bottom:22px;padding:18px 22px;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-md)}
.form-group{display:flex;flex-direction:column;gap:6px}
.form-group label{font-size:12px;font-weight:900;color:var(--muted);letter-spacing:.02em}
input[type=text],input[type=password],input[type=email],select,textarea{padding:11px 14px;border:1px solid var(--border);border-radius:14px;font-size:13px;color:var(--text);background:var(--surface-2);min-width:180px;outline:none;transition:border-color 160ms ease,box-shadow 160ms ease}
input[type=text]:focus,input[type=password]:focus,input[type=email]:focus,select:focus,textarea:focus{border-color:var(--oshi);box-shadow:0 0 0 4px color-mix(in srgb,var(--oshi) 18%,transparent)}
...
.modal{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:28px;width:540px;max-width:100%;box-shadow:var(--shadow);max-height:90vh;overflow-y:auto}
...
.toast{padding:14px 20px;border-radius:16px;font-size:13px;font-weight:800;color:#fff;box-shadow:0 16px 40px rgba(0,0,0,.22);animation:slideIn .22s ease both}
...
.role-pill{display:inline-flex;padding:5px 11px;border-radius:999px;font-size:11px;font-weight:900;background:var(--surface-2);border:1px solid var(--border);color:var(--muted)}

.kv-table{width:100%;border-collapse:collapse;font-size:13px}
.kv-table th{width:200px;text-align:left;padding:13px 16px;background:var(--surface-2);border-bottom:1px solid var(--border);font-weight:800;color:var(--muted);vertical-align:top;white-space:nowrap;text-transform:none;letter-spacing:0;font-size:12px}
...
@media (max-width:780px){
  .sidebar{position:static;width:100%;height:auto;border-right:0;border-bottom:1px solid var(--border);padding:16px}
